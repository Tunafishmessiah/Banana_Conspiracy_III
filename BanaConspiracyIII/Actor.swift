...
public class Actor
{
    ...
    func Actor(texture: String, _ scale: CGPoint) //CALL MAKETHINGSCOLLIDE IN ALL PLAYERS OR ENEMIES!
    {
        ...
        self.Body = SKPhysicsBody(rectangleOfSize: CGSizeMake(self.Node.size.width * scale.x, self.Node.size.height * scale.y))//Starting to make the body, the rest is up to the character/thing using it
